...
> **Experimental.** This action is not part of the 0.1.2 release. Bootstrap instructions currently reference a package that is not on npm, and the baseline does not survive the runner. Prefer the local proxy (`npx mcp-pin@0.1.2`). Do not adopt this action in CI yet.
